  ...
  useEffect(() => {
    const login = async () => {
      try {
        ...
        // Fetch question data
        axios.get('https://vietnam-history.azurewebsites.net/api/Question/getAllQuestions', config)
          .then(response => {
  ...
  useEffect(() => {
    const fetchAnswerData = async () => {
      ...
      for (const question of questionData) {
        try {
          const response = await axios.get(`https://vietnam-history.azurewebsites.net/api/Anwsers/getAllAnswersByQuestionId?id=${question.questionId}`, config);
          const answerItems = response.data.data;
          setAnswerData(prevAnswerData => [...prevAnswerData, { questionId: question.questionId, answerItems }]);
        } catch (error) {
        ...
        {questionData.map((question) => {
          ...
          return (
            <tr key={question.questionId}>
              ...
              <td className="td-user">{answer ? answer.answerItems.map(answer => answer.answerText).join(', ') : ''}</td>
